        ...
        .catch(err=>{
            ...
            if (err.response?.status === 400) {
                if (err.response?.data?.error?.includes('already exists')) {
                    showToast("warning", "", "A user with this email already exists. Please use a different email or try logging in.");
                } else {
                    showToast("error", "", `Registration failed: ${err.response?.data?.error || err.message}`);
                }
            } else {
